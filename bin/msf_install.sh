...
usage () {
	...
	echo ""
	echo "     	${RED}--reinstall 	WARNING: The script will ${BOLD}completely${NORMAL} ${RED}remove postgresql"
...
hash msfconsole >& /dev/null && [ $MSF_REINSTALL != 'y' ] && {
	...
	printf "${BOLD}\'--reinstall\'${NORMAL} ${RED}flag.${NORMAL}\n"
	echo ""
	usage
	exit 1
}
...
printf "${BLUE}[*] Checking Homebrew...${NORMAL}\n"
hash brew >& /dev/null || {
	printf "${RED}[!] Error: Homebrew is not installed.${NORMAL} Quitting...\n"
    exit 1
}
...
[ "${MSF_REINSTALL}" = "y" ] && {
	...
	printf "${YELLOW}[i] Removing MSF...${NORMAL}\n"
